...
# ----------------------------
# Optuna ハイパーパラメータチューニング関数
# ----------------------------
def objective(trial):
    gamma = trial.suggest_float('gamma', 0.5, 5.0, step=0.25)
    args = argparse.Namespace(hidden_channels=64, learning_rate=0.001, batch_size=32, epochs=1, weight_decay=5e-4, patience=30)
    main(args, gamma)
    # Assuming main returns validation loss
    return val_loss
