...
#         self.alpha = alpha  # Pass class_weights as alpha
#         self.gamma = gamma
...
#         # データの形状を確認するためのアサーション
#         assert inputs.dim() == 2, f"inputs must be a 2D tensor of shape (batch_size, num_classes), got {inputs.shape}"
#         assert targets.dim() == 1, f"targets must be a 1D tensor of shape (batch_size,), got {targets.shape}"
#         assert inputs.size(0) == targets.size(0), f"inputs and targets must have the same batch size, got inputs: {inputs.size(0)}, targets: {targets.size(0)}"

        
#         # if epoch % 20 == 0:
#         #     print(f"Softmax output at epoch {epoch}: {inputs[:1]}")  # Softmax適用確認
...
#         # if epoch % 100 == 0:
#         #     print(f"pt values at epoch {epoch}: {pt[:1]}")  # ptの値確認
        
#         focal_term = (1 - pt) ** self.gamma
#         alpha = self.alpha.to(targets.device)[targets]
#         loss = -alpha * focal_term * torch.log(pt)
...
def evaluate_and_visualize(test_loader, model, device, class_weights, test_pairs=None):
...
def main(args):
    ...
    if rank == 0:
        ...
        evaluate_and_visualize(final_test_loader, ddp_model, device, class_weights, test_pairs=test_pairs)
