...
def main(args):
    ...
    model = GCNModel(hidden_channels=args.hidden_channels, num_classes=10).to(device)
    ...
    for train_index, val_index in kf.split(pairs):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            # 修正: val_accuracy は全プロセスでの正しい総数から計算する
            # val_accuracy = total_correct.item() / total_samples*world_size
            val_accuracy = total_correct / total_samples /world_size



            if rank == 0:
                # 修正: predictions と labels を空でないか確認し、無効な状態を回避
    ...
    if rank == 0:
        ...
        # エラー防止：all_predsやall_labelsが空でないことを確認
        if all_preds and all_labels:
            ...
            print("Warning: No valid predictions or labels were generated in the test set.")
