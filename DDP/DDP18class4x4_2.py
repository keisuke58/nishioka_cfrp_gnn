...
def main(args):
    ...
    for train_index, val_index in kf.split(pairs):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            # 全プロセスで損失を平均
            avg_train_loss = torch.tensor(total_loss / len(train_loader)).to(device)
            ...
            # 検証損失と精度の平均
            avg_val_loss = torch.tensor(val_loss / len(val_loader)).to(device)
            dist.all_reduce(avg_val_loss, op=dist.ReduceOp.SUM)
            avg_val_loss /= world_size
    
            # 正解数の合計
            total_correct = torch.tensor(correct, dtype=torch.float).to(device)  # ここを修正
            dist.all_reduce(total_correct, op=dist.ReduceOp.SUM)
            total_correct /= world_size
    
            # 全サンプル数の合計
            total_samples_tensor = torch.tensor(total_samples, dtype=torch.float).to(device)  # 必要ならdtypeを指定
            ...
            if avg_val_loss.item() < best_val_loss:
                ...
                # モデルの保存（rank 0のみ）
                if rank == 0:
                    torch.save(ddp_model.module.state_dict(), f"best_model_fold_{fold}.pth")
            else:
                counter += 1
    ...
    if rank == 0:
        ...
        # モデルのロード（ベストモデル）
        model_path = f"best_model_fold_{all_fold_metrics[0]['fold']}.pth"  # 最初のフォールドのベストモデルを使用
...
if __name__ == '__main__':
    ...
    parser.add_argument('--patience', type=int, default=50, help='Early stopping patience')  # patienceを50に設定
