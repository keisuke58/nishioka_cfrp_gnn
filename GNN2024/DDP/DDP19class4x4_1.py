...
class GCNModel(torch.nn.Module):
    def __init__(self, hidden_channels=128, num_classes=19):
...
def evaluate_and_visualize(test_loader, model, device):
    ...
    # ROC AUC スコア (multi-class)
    try:
        roc_auc = roc_auc_score(all_labels, model(torch.tensor(all_preds)).cpu(), multi_class='ovr')
    except ValueError:
...
def main(args):
    ...
    model = GCNModel(hidden_channels=args.hidden_channels, num_classes=19).to(device)
    ...
    for train_index, val_index in kf.split(pairs):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % 10 == 0:
                print(f"\n--- Epoch {epoch} ---")
                # class_weightsはこのファイルでは使用されていないためコメントアウト
            ...
            if avg_val_loss.item() < best_val_loss:
                ...
                counter = 0
                if rank == 0:
                    model_save_path = f"/home/nishioka/GNN/GNNmodel/19classmodel/{type(model).__name__}_{timestamp}_best_model_fold_{fold}.pth"
    ...
    if rank == 0:
        ...
        model_path = f"/home/nishioka/GNN/GNNmodel/19classmodel/{type(model).__name__}_{timestamp}_best_model_fold_{all_fold_metrics[0]['fold']}.pth"
